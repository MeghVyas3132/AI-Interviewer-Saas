    ...
    try:
        ...
        for c in candidates:
            response_dict = {
                ...
                "status": c.status.value if hasattr(c.status, 'value') else str(c.status) if c.status else None,
                "source": c.source.value if c.source and hasattr(c.source, 'value') else str(c.source) if c.source else None,
