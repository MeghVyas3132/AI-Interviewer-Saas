...
import { getSubcategories, createSubcategory, updateSubcategory, deleteSubcategory } from '@/lib/local-data-store';

export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'GET') {
    try {
      const { examId } = req.query;
      const subcategories = await getSubcategories(examId as string);
    ...
    try {
      ...
      await updateSubcategory(id, { name, description });
    ...
    try {
      ...
      await deleteSubcategory(id as string);
