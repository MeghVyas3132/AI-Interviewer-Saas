...
async function postSignal(roundId: string, type: string, data: object) {
  ...
  console.log('[InterviewerMeeting] Sending signal:', { roundId, type, hasToken: !!authToken })
  try {
    ...
    if (!res.ok) {
      console.error('[InterviewerMeeting] postSignal failed:', res.status, await res.text())
    }
  } catch (err) {
...
async function postPresence(roundId: string) {
  ...
  try {
    ...
    const json = await res.json()
    console.log('[InterviewerMeeting] Presence posted, my_role:', json.role)
  } catch (err) {
...
async function checkPresence(roundId: string): Promise<boolean> {
  ...
  try {
    ...
    const json = await res.json()
    console.log('[InterviewerMeeting] Presence check:', { peer_online: json.peer_online, my_role: json.my_role, peer_role: json.peer_role })
    return json.peer_online || false
  } catch (err) {
    console.error('[InterviewerMeeting] checkPresence error:', err)
    return false
  }
}
...
export default function InterviewerMeeting({
  ...
  const retryConnection = useCallback(() => {
    if (retryCount >= MAX_RETRIES) {
      console.log('[InterviewerMeeting] Max retries reached')
      return
    }
    console.log(`[InterviewerMeeting] Retrying connection (${retryCount + 1}/${MAX_RETRIES})...`)
  ...
  useEffect(() => {
    ...
    pc.onconnectionstatechange = () => {
      ...
      if (pc.connectionState === 'failed') {
        ...
        if (retryCount < MAX_RETRIES) {
          console.log('[InterviewerMeeting] Connection failed, scheduling retry...')
          setTimeout(() => {
            retryConnection()
          }, 2000)
        }
  ...
  return (
    <div className="fixed inset-0 bg-gray-900 flex flex-col overflow-hidden">
      ...
      <div className="h-8 bg-gray-800/50 flex items-center justify-center px-4 text-xs text-gray-400">
        ...
        {peerOnline ? ' ✅ Candidate online' : ' ⏳ Waiting for candidate to join...'} |
