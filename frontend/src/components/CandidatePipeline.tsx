...
export function CandidatePipeline({ candidates }: PipelineProps) {
    const getCandidatesByStage = (stage: string) => {
        return candidates.filter(c => c.status.toLowerCase() === stage.toLowerCase());
    };
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            {STAGES.map(stage => {
                ...
                return (
                    <div key={stage.id} className="bg-gray-50 rounded-xl p-2 min-h-[300px] border border-gray-100">
                        {/* Header */}
                        ...
                        <div className="space-y-2">
                            {stageCandidates.map(candidate => (
                                <div key={candidate.id} className="bg-white p-3 rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition cursor-pointer group">
